...
class StarJunctionRecord:
    ...
    def key(self):
        ...
        between input files.
        """
        return (self.chromosome, self.intron_first, self.intron_last,
    ...
    def __iadd__(self, other):
        ...
        self.max_splice_overhang = max(self.max_splice_overhang,
                                       other.max_splice_overhang)
        return self

    def __str__(self):
        return "\t".join(map(str, [
            self.chromosome, self.intron_first, self.intron_last, self.strand,
            self.motif, self.annotation, self.n_unique_mapped,
            self.n_multi_mapped, self.max_splice_overhang]))
...
def process_files(args, logger):
    ...
    with writer(args.output, 'wt') as o:
        ...
        if len(args.input) > 1:
            ...
            logger.info("Writing merged STAR junction counts to {0}.".format(
                        args.output))
            # Merge and write
            ...
            logger.info("Only 1 STAR junction counts file provided. " +
                        "A new STAR junction counts file will be produced " +
                        "with a header line.")
            logger.info("Writing formatted STAR junction " +
                        "counts to {0}.".format(
                            args.output))
...
def main(args):
    ...
    logger.info("Merging/Formatting {0} STAR junction counts files.".format(
                len(args.input)))
